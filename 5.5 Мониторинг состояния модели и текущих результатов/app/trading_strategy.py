...
from app.metrics import app_metrics
        ...
        try:
            ...
            if analysis["signal"] in ["BUY", "SELL"] and analysis["signal"] != self.last_signal:
                ...
                response = requests.post(
                    f"http://{settings.TRADER_HOST}:{settings.TRADER_HOST_PORT}/trading/execute_trade",
    ...
    def start_live_trading(self, interval_seconds: int = 60) -> None:
        ...
        try:
            while self.is_running:
                ...
                response = requests.post(
                    f"http://{settings.TRADER_HOST}:{settings.TRADER_HOST_PORT}/trading/get_market_data",
